...
describe("POST /api/auth/signin", () => {
  ...
  it("redirects with a generic message when Supabase is unreachable", async () => {
    ...
    vi.mocked(createClient).mockReturnValue({
      auth: {
        signInWithPassword: vi.fn().mockRejectedValue(new Error("network unreachable")),
      },
  ...
  it("shows the verbatim Supabase message when credentials are rejected normally", async () => {
    const { context, redirect } = buildContext("climber@example.com", "wrong-password");
    vi.mocked(createClient).mockReturnValue({
      auth: {
    ...
    expect(redirect).toHaveBeenCalledWith(`/auth/signin?error=${encodeURIComponent("Invalid login credentials")}`);
  });
});
