...
| --- | -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------ | ------------- | ------------------------------------------------- |
| 1   | Bootstrap harness + critical failure-path coverage | Stand up a test runner and defend the two confirmed core-flow failures (login, plan generation) at the cheapest layer | #1, #3        | unit + integration + e2e | implementing | `context/changes/testing-critical-path-coverage/` |
...
| e2e                | Playwright      | `^1.63.0` — in use since `playwright-e2e-foundation`; authenticated pattern added §3 Phase 1 | login→questionnaire→plan seam smoke still pending (§3 Phase 2); not a per-input-combination tool                                                                                                                                                                                                                                        |
